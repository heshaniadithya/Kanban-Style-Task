...
function App() {
  ...
  const [{ isOver }, drop] = useDrop(() => ({
      accept: "todo",
      drop: (item) => addToCompleted(item.id, item.prjectName, item.taskDescription, item.timestamp, item.duration),
      collect: (monitor) => ({
        isOver: !!monitor.isOver(),
      })
  }))

  const addToCompleted = (id, prjectName, taskDescription, timestamp, duration) => {
    const moveTask = taskList.filter((task) => id === task.id);
    setCompleted((completed) => [...completed, {moveTask, prjectName, taskDescription, timestamp, duration}])
  }
  ...
  return (
    <>
      ...
      <h2 className="ml-6 text-xl font-semibold w-3/4 max-w-lg my-4 py-2 px-4 bg-gray-200">Completed:</h2>
